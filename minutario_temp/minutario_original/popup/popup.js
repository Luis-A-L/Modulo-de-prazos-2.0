document.addEventListener("DOMContentLoaded", function () {
  ...
  var saveUserBtn = document.getElementById("save-user-btn");
...
async function handleLogin(event) {
  ...
  try {
    ...
    if (userId) {
      var saveData = {
        minutario_user_id: userId,
        minutario_access_token: session.access_token,
        minutario_refresh_token: session.refresh_token,
      };
      // Salva email se checkbox marcado
      var saveCheck = document.getElementById("save-user-check");
      if (saveCheck && saveCheck.checked) {
        saveData.minutario_saved_email = email;
      } else {
        saveData.minutario_saved_email = null;
      }
...
function saveUser() {
  ...
  if (!emailVal) {
    setWordProbeStatus("Nenhum email para salvar.", true);
    return;
  }
  chrome.storage.local.set({ minutario_saved_email: emailVal }, function () {
    setWordProbeStatus("Email salvo!");
  });
}
