...
function getUserIdFromUser(user) {
  ...
  async function handleDeleteTemplate() {
    ...
    try {
      ...
      // Atualiza a lista local sem recarregar do IndexedDB
      if (existing) {
        allTemplates = allTemplates.map(function(t) { return t.id === tpl.id ? tpl : t; });
      } else {
      ...
      if (userId && window.MinutarioSync && window.MinutarioSync.syncTemplates) {
        window.MinutarioSync.syncTemplates(userId).then(function() {
