(function (global) {
  ...
  async function push(templates, folders) {
    ...
    if (folderRows.length > 0) {
      await sb.from(_config.FOLDERS_TABLE || "minutario_folders").upsert(folderRows, { onConflict: "id" });
    }
    ...
    if (templateRows.length > 0) {
      await sb.from(_config.TEMPLATES_TABLE || "minutario_templates").upsert(templateRows, { onConflict: "id" });
    }
  ...
  async function pull() {
    ...
    var templatesResult = await sb.from(_config.TEMPLATES_TABLE || "minutario_templates").select("*").eq("user_id", userId);
    if (templatesResult.error) return { success: false, error: templatesResult.error.message };

    var foldersResult = await sb.from(_config.FOLDERS_TABLE || "minutario_folders").select("*").eq("user_id", userId);
