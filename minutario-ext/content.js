(function (global) {
  ...
  chrome.runtime.onMessage.addListener(function (message, sender, sendResponse) {
    ...
    if (message && message.type === "GET_SUPABASE_TOKEN") {
      try {
        ...
        if (key) {
          ...
          if (parsed && parsed.access_token) {
            sendResponse({ ok: true, access_token: parsed.access_token, refresh_token: parsed.refresh_token, email: (parsed.user && parsed.user.email) || "" });
            return true;
          }
        }
      } catch(e) {}
  ...
  function proactivelySyncSession() {
    try {
      var key = Object.keys(localStorage).find(function(k) { return k.startsWith("sb-") && k.endsWith("-auth-token"); });
      if (key) {
        var raw = localStorage.getItem(key);
        var parsed = JSON.parse(raw);
        if (parsed && parsed.access_token && parsed.user && parsed.user.email && parsed.user.email.endsWith("@tjpr.jus.br")) {
  ...
  // Tenta sincronizar a sessão proativamente ao carregar a página
  setTimeout(proactivelySyncSession, 1500);
