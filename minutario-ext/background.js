...
async function performSync() {
  try {
    const stored = await chrome.storage.local.get(["minutario_user_id", "minutario_access_token", "minutario_refresh_token"]);
    ...
    // Restaura sessão Supabase no background
    if (stored.minutario_access_token && stored.minutario_refresh_token) {
      try {
        const client = typeof MinutarioAPI !== "undefined" && MinutarioAPI.getClient ? MinutarioAPI.getClient() : null;
        if (client) {
          await client.auth.setSession({
            access_token: stored.minutario_access_token,
            refresh_token: stored.minutario_refresh_token,
          });
        }
      } catch (e) {
  ...
  void (async () => {
    ...
    try {
      switch (message?.type) {
        ...
        case "UPDATE_SESSION": {
          if (message.payload && message.payload.access_token) {
            await chrome.storage.local.set({
              minutario_access_token: message.payload.access_token,
              minutario_refresh_token: message.payload.refresh_token,
              minutario_user_id: message.payload.user_id
            });
            performSync().catch(() => {});
